...
## Continuing project — LoomLogic (training instance)

This brain was **seeded from** live `.cursor/brain`. North star and stack are **already known**.

- **Skip** Q1 north-star rewrite and full ARCH stack shopping unless human asks to revisit.
- **Do** interview for Brainiac gaps: devices, deploy, success metric, NFRs, out-of-scope, CLIENT conflicts.
- **Do** continue D1 for journeys still `draft` (J4–J9) when human asks.
- Stack lock lives in DECISIONS + loomlogic-core (React / FastAPI / Postgres RLS) — profile ≈ **Multi-tenant SaaS**.

**Trigger (mechanical emptiness):** any of:
